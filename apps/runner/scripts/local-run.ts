...
//     config.ts) to point at /tmp/boop-runner/{repo,skills}.
...
// the disk cost of the source files.
...
// The wrapper lives at apps/runner/scripts/local-run.ts. The skill
...
if (missing.length > 0) {
  ...
  console.error(
    "[local-run] see apps/runner/scripts/local-run.ts for the full env contract.",
  );
  process.exit(2);
}
...
if (!env.OPENROUTER_API_KEY) {
  try {
    await fs.access(env.BOOP_OPENROUTER_API_KEY_PATH as string);
  } catch {
    console.error(
  ...
  const keyPath = env.BOOP_OPENROUTER_API_KEY_PATH as string;
  await fs.mkdir(path.dirname(keyPath), {
    recursive: true,
    mode: 0o700,
  });
  ...
  await fs.writeFile(keyPath, env.OPENROUTER_API_KEY + "\n", {
    mode: 0o400,
  });
}
...
const headOut = await execFile("git", ["rev-parse", "HEAD"], {
