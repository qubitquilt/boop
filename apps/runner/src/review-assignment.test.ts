...
// into `lib/workflow.ts` and the assignment is `state.review = ...`
...
// QUB-136: the runner is now TypeScript; the source-of-truth for
// the regression check is the .ts files (the compiled .js is
// generated by tsc, not authored by hand).
const indexSrc = readFileSync(path.join(here, "index.ts"), "utf8");
const workflowSrc = readFileSync(path.join(here, "lib", "workflow.ts"), "utf8");

test("review-skill call assigns to a declared binding", () => {
  // Find the file that has the review-skill call.
  const callIdxByFile = [
    ["index.ts", indexSrc],
    ["lib/workflow.ts", workflowSrc],
  ].map(([label, src]) => [
    label,
    src,
    Math.max(
  ...
  assert.ok(
    found,
    "no `await runOpenCodeSkill(` or `await skillFn(` call found in index.ts or lib/workflow.ts",
  );
