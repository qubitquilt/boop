...
// orchestrator in index.ts drives).
...
test("runStages walks every macro stage in order", async () => {
  ...
  // github.ts side effects of summary + inlines are pushed
...
test("runStages skips summary + inlines when review has no summary", async () => {
  ...
  // orchestrator in index.ts will surface the dashboard
...
test("runStages threads state between stages (handshake → fetch → sniff)", async () => {
  ...
  // index.ts; this test pins the keys.
...
test("a failed gate sets state.parseFailed and short-circuits the run (QUB-90)", async () => {
  ...
  // index.ts checks state.parseFailed and short-circuits
...
test("classify sub-stage calls overrides.classify and writes state.classification (QUB-94)", async () => {
  ...
  // default falls back to the stub in lib/classify.ts; a
      ...
      runSubWorkflow(
        ...
        recordingDeps(),
        {
          ...
          runExperts: async (names) => {
            ...
            const realExperts = await import("./experts.ts");
...
test("runStages aborts when state.passed already has a macro stage (QUB-102)", async () => {
  ...
  // The orchestrator in index.ts reads state.parseFailed and
...
test("runStages aborts even when state.passed only has the summary stage (QUB-102)", async () => {
  ...
  // merge in index.ts may shuffle the order (the loop hits
...
test("runStages calls onStagePassed after every macro stage (QUB-92)", async () => {
  // The orchestrator (index.ts) uses the onStagePassed
...
//   1. workflow.ts exports statusStageFor(id) and the
//      STAGES.statusStage field; tests pin the mapping.
//   2. github.ts exports STATUS and SHORT maps; tests pin
...
//   3. The integration test in index.test.ts already asserts
...
// orchestrator in index.ts posts directly). The mapping is
// enumerated below.
...
test("every status header line (STATUS map) carries the same emoji (QUB-93)", async () => {
  ...
  const { STATUS } = await import("./github.ts");
