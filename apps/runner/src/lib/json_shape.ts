...
// parsers (parseExpertResponse in experts.ts, parseReviewOutput
// in openrouter/parser.ts) all share the same JSON-fence shape;
 ...
 */
export function stripJsonFence(text: string | null | undefined): string {
  if (!text) return "";
  return String(text)
    .replace(/^```(?:json)?\s*/i, "")
    .replace(/```\s*$/, "")
