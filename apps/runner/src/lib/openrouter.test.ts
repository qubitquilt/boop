...
import { LENS_FILES, loadConfig } from "./config.ts";
...
test("parseReviewOutput accepts a real review with required structure", () => {
  ...
  const out = [
    ...
    "- The 2 new Q-N tests in `openrouter.test.ts` round-trip the structured block in both summary and inline positions.",
    "",
...
test("buildBoopPrompt source preserves H5 ordering invariant", () => {
  ...
  // QUB-136: the prompt builder is now openrouter/prompt.ts; the
  // helper functions are factored out, so we strip the leading
  // comment block and look for the runtime string literals in the
  // function body. The file header mentions both markers in a
  // docstring, so we have to skip the leading // comment lines.
  const src = readFileSync(
    fileURLToPath(new URL("./openrouter/prompt.ts", import.meta.url)),
    "utf8",
  );
  // Strip leading // comment lines (file header).
  const codeOnly = src.replace(/^\/\/[^\n]*\n/gm, "");
  const systemIdx = codeOnly.indexOf('"## SYSTEM INSTRUCTIONS (authoritative)"');
  const dataIdx = codeOnly.indexOf('"## DATA (PR-controlled');
  assert.ok(systemIdx > -1, "SYSTEM INSTRUCTIONS marker missing");
  assert.ok(dataIdx > -1, "DATA marker missing");
  assert.ok(systemIdx < dataIdx, "SYSTEM must come before DATA");
});
...
// isToolsEnabled lives in tools.ts as toolsAvailable, which is the
// single gate shared by the prompt builders and the tool factory
// (see buildAgentTools in tools.test.ts).
