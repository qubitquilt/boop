...
import { parseReviewOutput } from "./openrouter.ts";
...
test("runExperts concatenates findings from every expert", async () => {
  ...
  // in lib/walkthrough.test.ts and the real-expert
...
// production deps (built by index.ts's makeDeps) trigger
// "deps.callOpenRouter is not a function" on every expert
// dispatch. The walkthrough stage has the same pattern
// (walkthrough.ts:128); this test pins the experts half so
...
// (openrouter.ts:44); the expert dispatch inherits the same
...
// (workflow.ts:590) and threads it through deps.env
// (workflow.ts:591-600). The expert dispatch spreads ...deps
// into callOpenRouter (experts.ts:233), so deps.env lands in
// the SDK call. Without this, callOpenRouter defaults to
// process.env (openrouter.ts:172) and the key is unset — the
// runner never exports the file-loaded secret into the
// process environment — so the guard at openrouter.ts:188-191
