...
import { createRtkAdapter } from "./rtk.ts";
...
test("createRtkAdapter: routes through rtk read with the expected flags", async () => {
  // which returns a path; the smoke test (against /dev/null) returns
  // empty stdout/stderr; rtk read returns compressed content. The
  // trailing newline is what `rtk read` actually emits; the
  ...
  // 1) which rtk, 2) rtk smoke test, 3) rtk read.
  assert.equal(calls.length, 3);
  // The smoke test runs rtk read against /dev/null so the smoke
  // test mirrors the production CLI shape (read + --truncate-lines-at
  // + path) — a binary that fails on `--` fails the smoke test
  // before any real read is attempted.
  const smokeCall = calls[1];
  assert.equal(smokeCall.bin, "/usr/local/bin/rtk");
  ...
  // site controls the path). The adapter does NOT pass
  // `--max-lines` — the line cap lives in the rtk filter
  // (apps/runner/rtk/filters.toml) so the cap is the same for
  // every call site, not duplicated in every CLI invocation.
  const rtkCall = calls[2];
...
test("createRtkAdapter: per-call truncateLinesAt overrides the default", async () => {
  const { calls, exec } = recordingExecFile([
    { stdout: "/usr/local/bin/rtk\n" },
    { stdout: "", stderr: "" },
    { stdout: "compressed\n" },
  ...
  // Calls: 1) which rtk, 2) rtk smoke test, 3) the real read.
  const rtkCall = calls[2];
  assert.deepEqual(rtkCall.args, [
    "read",
    "--truncate-lines-at",
...
test("createRtkAdapter: rtk read failure flips to raw mode for the rest of the run", async () => {
  ...
  // and the smoke test passes (returns empty stdout/stderr on
  // /dev/null), but a real read call throws (e.g. the installed
  // rtk rejects the ConfigMap mount path with "No such file or
  // directory"). The first failure must:
  ...
  // rtk was invoked twice: the smoke test (passed → source
  // stays "rtk") plus the one real read that throws. The
  // failure flips source to "raw"; the next two reads bypass
  // the binary. So rtkInvocations.length === 2, not 3.
  const rtkInvocations = calls.filter((c) => c.bin === "/usr/local/bin/rtk");
  assert.equal(rtkInvocations.length, 2, "rtk should be called twice (smoke + first read)");
});

test("createRtkAdapter: smoke-test failure disables rtk up front (no per-read errors)", async () => {
  // The macOS homebrew rtk 0.42.3 bug: a successful `which rtk`
  // resolves the binary, but `rtk read -- /path` internally
  // shells out to bash's `read` builtin which fails on `--`
  // (`read: --: invalid option`). Without a smoke test, every
  // read attempt fails the same way and the operator sees
  // 5+ noise lines per run. With the smoke test, init()
  // detects the failure and flips the adapter to raw mode
  // before any real read is attempted.
  const { calls, exec } = recordingExecFile([
  ...
  const fs = fakeFs({
    "/skill/boop/SKILL.md": "raw skill body",
    "/skill/boop/agents/review-design-pattern.md": "raw lens body",
  });
  const logCalls = [];
  const adapter = createRtkAdapter({
    execFile: exec,
  ...
  // Every read goes straight to fs.readFile. rtk is never called
  // for a real read; only the smoke test fires rtk.
  assert.equal(await adapter.readFile("/skill/boop/SKILL.md"), "raw skill body");
  assert.equal(await adapter.readFile("/skill/boop/agents/review-design-pattern.md"), "raw lens body");
  // Exactly one rtk invocation: the smoke test. The two reads
  // bypass rtk entirely because the smoke test failed.
  ...
  // State reports raw mode with the smoke-test-failed reason so
  // the operator can see why the adapter is bypassing rtk.
  const state = await adapter.init();
  assert.equal(state.source, "raw");
  assert.match(state.reason ?? "", /smoke-test-failed/);
  // The single fallback log fires once on the first read, not
  // once per read.
  const fallbackLogs = logCalls.filter((l) =>
    l.msg.startsWith("falling back to raw read"),
  );
...
test("createRtkAdapter: init() is idempotent and memoised", async () => {
  // The which + smoke-test calls happen once; subsequent init
  // calls reuse the captured state. Without the memoisation,
  // every read would re-spawn which.
  const { calls, exec } = recordingExecFile([
    { stdout: "/usr/local/bin/rtk\n" },
    { stdout: "", stderr: "" },
  ]);
  ...
  assert.equal(a, b);
  assert.equal(b, c);
  assert.equal(calls.length, 2, "init() should memoize which + smoke test");
});
